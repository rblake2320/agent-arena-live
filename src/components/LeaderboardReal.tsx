...
import { Trophy, TrendingUp, Medal, Crown, ChevronRight, Wifi, WifiOff } from 'lucide-react';
import { Button } from './ui/button';
import { useRealtimeLeaderboard } from '../hooks/useRealtimeLeaderboard';
import { formatStreak, formatRatingChange, calculateWinRate } from '../lib/services/teams';
...
export const LeaderboardReal: React.FC = () => {
  const { teams, isLoading, error, isSubscribed } = useRealtimeLeaderboard(10);
            ...
            {isLoading ? (
              ...
              <>
                {[1, 2, 3, 4, 5].map((i) => (
                  <div
                    key={i}
                    className="grid grid-cols-[auto,1fr,auto,auto,auto] md:grid-cols-[60px,1fr,100px,100px,80px,80px] gap-4 p-4 items-center border-b border-border/30 animate-pulse"
                  >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="w-24 h-5 bg-muted rounded mb-1"></div>
                        <div className="w-16 h-4 bg-muted rounded"></div>
                      </div>
                    </div>
                    <div className="hidden md:block w-12 h-5 bg-muted rounded mx-auto"></div>
                    <div className="hidden md:block w-16 h-5 bg-muted rounded mx-auto"></div>
                    <div className="w-10 h-5 bg-muted rounded mx-auto"></div>
                    <div className="w-8 h-5 bg-muted rounded mx-auto"></div>
                  </div>
                ))}
              </>
            ) : teams.length === 0 ? (
              ...
              teams.map((team, index) => (
                ...
                >
                  ...
                  <div className="flex justify-center">
                    <div
                      className={`w-10 h-10 rounded-lg bg-gradient-to-br ${getRankColors(
                        team.rank
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">
                        {team.owner?.username || team.owner?.display_name || 'Unknown'}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-center font-display font-bold text-neon-cyan hidden md:block">
                    {team.rating?.toLocaleString() || 1500}
                  </div>

                  {/* W/L */}
                  <div className="text-center text-sm hidden md:block">
                    <span className="text-neon-green">{team.wins || 0}</span>
                    <span className="text-muted-foreground">/</span>
                    <span className="text-destructive">{team.losses || 0}</span>
                    <div className="text-xs text-muted-foreground">
                      {calculateWinRate(team)}% WR
                    </div>
                  </div>
                  ...
                  {/* 24h Change - Mock data for now, would be calculated from rating_history */}
                  <div className="text-center">
